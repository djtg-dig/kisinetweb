...
export default function EditProductPage({ params }: EditPageProps) {
  ...
  useEffect(() => {
    ...
    async function loadProduct() {
      ...
      try {
        const product = await getProductDetail(pharmacyId, reference);
        setValues({
          name: product.name,
          description: product.description || "",
          form: product.form || initialProductFormValues.form,
          target_gender: product.target_gender || initialProductFormValues.target_gender,
          target_age_group:
            product.target_age_group || initialProductFormValues.target_age_group,
          therapeutic_category:
            product.therapeutic_category || initialProductFormValues.therapeutic_category,
    ...
    <>
      <main className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <section className="py-8">
          {loadState === "loading" && (
            <p className="text-sm text-app-muted">Chargement du produit...</p>
          )}
